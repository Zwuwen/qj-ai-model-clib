...
RknnRet cgdd_engine::Inferenct(cv::Mat &srcimg,cv::Mat &inputImg,detect_result_group_t *detect_result_group,char* taskID)
{
	try
	{
		if(srcimg.empty() || inputImg.empty() || detect_result_group == NULL || m_alg_impl == NULL ) {
			printf("param error\n");
			return RKNN_ERR;
			}
		...
		TransferBuffer* tp=new TransferBuffer(std::string("0"),srcimg.rows,srcimg.cols,srcimg.data,0);
		...
		{// 分类模型
			ClassifyRes* classify_res =  (ClassifyRes*)alg_res.data_.get();
		...
		{// 目标检测模型
			...
			for(std::vector<DetectRes>::iterator it = detect_res.begin(); it < detect_res.end(); it++ )
			{
				if(it->class_id_ != -1)
				{
